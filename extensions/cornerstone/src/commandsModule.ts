...
import {
  getAINoduleAnnotationUID,
  getNoduleSeriesUID,
  getNoduleRectanglePoints,
  getAINoduleRiskColor,
...
function commandsModule({
  ...
  /**
   * Hydrates AI nodule bounding boxes into the standard Cornerstone rectangle
   * annotation state. The command is data-source agnostic so other AI backends
   * can provide the same result shape.
   */
  function displayAINoduleDetections({
    detections,
    studyInstanceUID,
    seriesInstanceUID,
  }: {
    ...
    const nodules = parseAINoduleResults(detections);
    if (!nodules.length) {
      return { total: 0, applied: 0, skipped: 0 };
    }

    const activeDisplaySets = displaySetService.getActiveDisplaySets?.() || [];
    const displaySets = activeDisplaySets.filter(displaySet => {
      if (studyInstanceUID && displaySet.StudyInstanceUID !== studyInstanceUID) {
        return false;
      }
    ...
    nodules.forEach(({ id, nodule }) => {
      const selection = selectNoduleImage(nodule, displaySets);
      const imageId =
        selection?.image?.imageId || selection?.displaySet?.imageIds?.[selection.imageIndex];
      if (!selection || !imageId) {
        skipped += 1;
        return;
      }

      const viewportId = viewportIds.find(candidateViewportId => {
        const viewportInfo = cornerstoneViewportService.getViewportInfo(candidateViewportId);
        if (!viewportInfo) {
          return false;
        }
        try {
          return viewportInfo.contains(selection.displaySet.displaySetInstanceUID, imageId);
        } catch (_error) {
          return false;
        }
      });
      ...
      const annotationUID = getAINoduleAnnotationUID(
        studyInstanceUID,
        id,
        selection.displaySet.SeriesInstanceUID
      );
      ...
      const points = getNoduleRectanglePoints(nodule, selection.image);
      if (!points) {
        skipped += 1;
        return;
      }

      cornerstoneTools.RectangleROITool.hydrate(viewportId, points as any, {
        annotationUID,
        referencedImageId: imageId,
      });

      const annotationInstance = annotation.state.getAnnotation(annotationUID);
      if (!annotationInstance) {
        skipped += 1;
        return;
      }

      annotationInstance.autoGenerated = true;
      (annotationInstance as any).metadata = {
        ...annotationInstance.metadata,
        aiNodule: nodule,
        aiNoduleId: id,
        aiRiskLevel: nodule.risk_level,
      };
      annotationInstance.data.label = '';

      const color = getAINoduleRiskColor(nodule.risk_level);
      annotation.config.style.setAnnotationStyles(annotationUID, {
        color,
        colorAutoGenerated: color,
        colorLocked: color,
        lineWidth: '2',
        lineWidthAutoGenerated: '2',
        lineWidthLocked: '2',
        textBoxVisibility: false,
        textBoxVisibilityAutoGenerated: false,
        textBoxVisibilityHighlighted: false,
        textBoxVisibilityLocked: false,
        textBoxVisibilitySelected: false,
      });
    ...
    return { total: nodules.length, applied, skipped };
  }

  function clearAINoduleDetections() {
    const annotations = annotation.state.getAllAnnotations?.() || [];
    annotations
      .filter(annotationInstance => (annotationInstance as any).metadata?.aiNodule)
      .forEach(annotationInstance => {
        annotation.locking.setAnnotationLocked(annotationInstance.annotationUID, false);
        annotation.state.removeAnnotation(annotationInstance.annotationUID);
      });
    cornerstoneViewportService.getRenderingEngine?.()?.render?.();
  }

  /** Loads the nodule's series into a compatible viewport and navigates to its source image. */
  async function navigateToAINodule({
    record,
    nodule,
    studyInstanceUID,
  }: {
    ...
    const targetNodule = record?.nodule || nodule;
    const targetStudyInstanceUID = record?.studyInstanceUID || studyInstanceUID;
    if (!targetNodule) {
      return false;
    }
    ...
    const findSelection = () => {
      const displaySets = (displaySetService.getActiveDisplaySets?.() || []).filter(
        displaySet =>
          !targetStudyInstanceUID || displaySet.StudyInstanceUID === targetStudyInstanceUID
      );
      return selectNoduleImage(targetNodule, displaySets);
    };

    let selection = findSelection();
    if (!selection && (record?.seriesInstanceUID || getNoduleSeriesUID(targetNodule))) {
      selection = await new Promise<ReturnType<typeof findSelection>>(resolve => {
        let timeoutId;
        const subscription = displaySetService.subscribe(
          displaySetService.EVENTS.DISPLAY_SETS_ADDED,
          () => {
            const nextSelection = findSelection();
            if (nextSelection) {
              clearTimeout(timeoutId);
              subscription.unsubscribe();
              resolve(nextSelection);
            }
          }
        );
    ...
    const targetDisplaySetUID = selection.displaySet.displaySetInstanceUID;
    const displayedViewport = [...viewports.values()].find(viewport =>
      viewport.displaySetInstanceUIDs?.includes(targetDisplaySetUID)
    );
    const viewportId = displayedViewport?.viewportId || activeViewportId;
    if (!viewportId) {
      return false;
    }
    ...
    const navigateViewport = () => {
      ...
      if (referencedImageId) {
        viewport.setViewReference({ referencedImageId });
      } else {
        csUtils.jumpToSlice(viewport.element, { imageIndex: selection.imageIndex });
      }
      viewport.render();
      return true;
    };

    if (displayedViewport && navigateViewport()) {
      return true;
    }

    let subscription;
    let timeoutId;
    const viewportReady = new Promise<boolean>(resolve => {
      ...
      subscription = cornerstoneViewportService.subscribe(
        cornerstoneViewportService.EVENTS.VIEWPORT_DATA_CHANGED,
        ({ viewportId: changedViewportId }) => {
          if (changedViewportId === viewportId) {
            finish(navigateViewport());
          }
        }
      );
      timeoutId = setTimeout(() => finish(navigateViewport()), 5000);
    });

    const cineState = cineService.getState();
    const currentCineState = cineState.cines?.[viewportId];
    cineService.setCine({
      id: viewportId,
  ...
  const actions = {
    displayAINoduleDetections,
    clearAINoduleDetections,
    navigateToAINodule,
  ...
  const definitions = {
    displayAINoduleDetections: {
      commandFn: actions.displayAINoduleDetections,
    },
    clearAINoduleDetections: {
      commandFn: actions.clearAINoduleDetections,
    },
    navigateToAINodule: {
      commandFn: actions.navigateToAINodule,
    },
